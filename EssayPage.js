...
if(!user){dialog('User Not Found','Email not registered.','bx-user-x','#ff4757');return}
var profile=user.Profile||{};var plan=user.Plan||{};var agora=Math.floor(Date.now()/1000);var duracao=plan.Duracion;
if(profile.Status==='Banned'){dialog('Account Banned','Permanently suspended.','bx-block','#ff4757');return}
if(profile.Status==='Disabled'){dialog('Account Disabled','Contact administrator.','bx-minus-circle','#febc2e');return}
if(profile.Status==='Expired'){dialog('Subscription Expired','Renew to continue.\n\n'+fmtT(duracao),'bx-time-five','#ff4757');return}
...
fetch(fb+'/.json?auth='+ak,{method:'PATCH',body:JSON.stringify(nd)}).then(function(){buildPage(user)});
return;
}
if(profile.Uid!==deviceId){dialog('Device Not Authorized','Linked to another device.','bx-devices','#ff4757');return}
