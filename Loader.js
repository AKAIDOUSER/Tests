...
function limparTema(t){t=t.replace(/["""]/g,'').replace(/\*\*/g,'').replace(/##/g,'').trim();t=t.replace(/^TEMA:\s*/i,'').replace(/^Tema:\s*/i,'').trim();var idx=t.indexOf(' - ');if(idx>0)t=t.substring(0,idx).trim();t=t.replace(/\s*\d{4}$/,'').replace(/\s*\d+ª\s*série.*$/i,'').replace(/\s*\d+º\s*tri.*$/i,'').trim();t=t.replace(/TEMA:\s*/gi,'').replace(/Tema:\s*/g,'').trim();return t}
