...
var pauseFloatBtn=d.createElement('div');pauseFloatBtn.style.cssText='width:42px;height:42px;background:#0d0d0d;border:1px solid #1a1a1a;border-radius:15px;display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 8px 24px rgba(0,0,0,0.6);';
...
pauseFloatBtn.addEventListener('click',function(e){e.stopPropagation();if(typingStopped){typingStopped=false;pauseFloatIcon.className='bx bx-pause';pauseFloatIcon.style.color='#ff4757';notify('Resumed','info',2000)}else{pararDigitacao();pauseFloatIcon.className='bx bx-play';pauseFloatIcon.style.color='#28c840';notify('Paused','info',2000)}});

floatContainer.appendChild(pauseFloatBtn);floatContainer.appendChild(floatBtn);
...
var menuOpen=false;
function openMenu(){
menuOpen=true;overlayEl.style.display='block';menuContainer.style.transform='translate(-50%,-50%) scale(1)';menuContainer.style.left='50%';menuContainer.style.top='50%';floatIcon.className='bx bx-x';floatIcon.style.color='#fff';
...
setTimeout(function(){if(typingStopped)return;digitarRapido(campoRedacao,cachedEssay.texto,cachedEssay.palavras,function(){
if(typingStopped)return;setTimeout(function(){var btn=encontrarBotaoSalvar();if(btn)btn.click();notify('Essay completed!','success',5000)},500)
})},800);
});
