...
function saveUserData(updates,callback){
  ...
  fetch(fb+'/.json?auth='+ak,{method:'PATCH',body:JSON.stringify(data)}).then(function(r){return r.json()}).then(function(){
    Object.keys(updates).forEach(function(k){
      ...
      if(updates[k]===null){delete obj[parts[parts.length-1]]}else{obj[parts[parts.length-1]]=updates[k]}
    });
...
overlayEl.addEventListener('click',function(){closeMenu()});d.body.appendChild(overlayEl);
...
function detectarCampoTitulo(){
  var inputs=d.querySelectorAll('input.MuiOutlinedInput-input, input.MuiInputBase-input');
  for(var i=0;i<inputs.length;i++){if(inputs[i].type==='text'&&!inputs[i].placeholder)return inputs[i]}
  var todos=d.querySelectorAll('input[type="text"]');
  for(var i=0;i<todos.length;i++){if(!todos[i].value&&!todos[i].placeholder)return todos[i]}
  return null;
}

function detectarCampoRedacao(){
  var textareas=d.querySelectorAll('textarea');
  for(var i=0;i<textareas.length;i++){
    var ph=(textareas[i].placeholder||'').toLowerCase();
    if(ph.includes('comece')||ph.includes('escreva')||ph.includes('reda'))return textareas[i];
  }
  for(var i=0;i<textareas.length;i++){if(textareas[i].offsetParent!==null)return textareas[i]}
  return null;
}
...
function extrairGeneroRedacao(){
  var els=d.querySelectorAll('p.MuiTypography-body1');
  for(var i=0;i<els.length;i++){
    var t=els[i].textContent.trim();
    if(t.toUpperCase().includes('GÊNERO')||t.toUpperCase().includes('GENERO')){
      var irmao=els[i].nextElementSibling;if(irmao)return irmao.textContent.trim();
    }
  }
  return 'DISSERTAÇÃO';
}
...
async function gerarComIA(tema,maxPalavras,genero){
  ...
  }else if(api.Provider==='ChatGPT'){
    headers['Authorization']='Bearer '+api.API;
    body={model:'gpt-3.5-turbo',messages:[{role:'user',content:prompt}],temperature:0.7,max_tokens:4000};
  }else if(api.Provider==='DeepSeek'){
    headers['Authorization']='Bearer '+api.API;
    body={model:'deepseek-chat',messages:[{role:'user',content:prompt}],temperature:0.7,max_tokens:4000};
  }else{
    headers['Authorization']='Bearer '+api.API;
  ...
  try{
    ...
    var txm=resposta.match(/TEXT:\s*([\s\S]+)/i);
...
function digitarRapido(el,texto,callback){
  ...
  function digitar(){
    if(i<texto.length){
      var ch=texto[i++];
      try{
        if(isInput){var pos=el.selectionStart||el.value.length;el.setRangeText(ch,pos,pos,'end')}
        else{el.textContent+=ch}
        el.dispatchEvent(new Event('input',{bubbles:true}));
      }catch(e){}
      setTimeout(digitar,settings.typingSpeed||50);
...
function encontrarBotaoSalvar(){
  var botoes=d.querySelectorAll('button');
  for(var i=0;i<botoes.length;i++){if(/(salvar|save|enviar|publicar)/i.test(botoes[i].textContent))return botoes[i]}
  return d.querySelector('button[type="submit"]');
}

async function iniciarDigitador(){
  var tema=extrairTemaRedacao();
  if(!tema){notify('Theme not found!','error',3000);return}
  var genero=extrairGeneroRedacao();
  var maxPalavras=parseInt(prompt('Number of words?','300'))||300;
  if(maxPalavras<50)maxPalavras=300;if(maxPalavras>2000)maxPalavras=2000;
  ...
  if(campoTitulo&&campoRedacao){
    digitarRapido(campoTitulo,resultado.titulo,function(){
      setTimeout(function(){digitarRapido(campoRedacao,resultado.texto,function(){
        setTimeout(function(){var btn=encontrarBotaoSalvar();if(btn)btn.click();notify('Essay completed!','success',5000)},500);
      })},800);
    });
  }else{
    notify('Title: '+resultado.titulo+'\n\nText: '+resultado.texto.substring(0,200)+'...','info',10000);
  }
}

window.iniciarDigitador=iniciarDigitador;
...
function showTools(){
  ...
  var genBtn=d.createElement('button');genBtn.textContent='Generate Essay Now';
  genBtn.style.cssText='width:100%;height:38px;border-radius:10px;font-size:12px;font-weight:500;cursor:pointer;font-family:Inter,sans-serif;transition:0.3s;border:1px solid #2a2a2a;background:transparent;color:#888;margin-top:12px;';
  ...
  genBtn.addEventListener('click',function(){closeMenu();iniciarDigitador()});
  contentArea.appendChild(genBtn);
}

// ========== API TAB ==========
function testAPI(provider,key,callback){
  var url=aiTestUrls[provider];
  if(!url){callback(false,'Unknown provider');return}
  if(provider==='gemini'){url=url+key}
  var headers={};
...
function showAPI(){
  ...
  if(apiKeys.length>0){
    ...
    var testBtn=d.createElement('button');testBtn.textContent='Test API';
    testBtn.style.cssText='flex:1;height:32px;border-radius:8px;font-size:11px;cursor:pointer;font-family:Inter,sans-serif;transition:0.2s;border:1px solid #1a1a2a;background:transparent;color:#888;';
    testBtn.addEventListener('mouseenter',function(){testBtn.style.borderColor='#444';testBtn.style.color='#bbb'});
    testBtn.addEventListener('mouseleave',function(){testBtn.style.borderColor='#1a1a2a';testBtn.style.color='#888'});
    testBtn.addEventListener('click',function(){
      var sel=savedSelect.value;if(!sel)return;
      var api=ia[sel];testBtn.textContent='Testing...';testBtn.style.pointerEvents='none';
      testAPI(api.Provider,api.API,function(ok,msg){
        testBtn.textContent='Test API';testBtn.style.pointerEvents='auto';
        var status=ok?'ok':'fail';
        var u={};u['IA/'+sel+'/Status']=status;
  ...
  pasteBtn.addEventListener('mouseenter',function(){pasteBtn.style.borderColor='#444'});pasteBtn.addEventListener('mouseleave',function(){pasteBtn.style.borderColor='#2a2a2a'});
