...
function calcExp(dias){
  var d=parseInt(dias);if(isNaN(d)||d<1)return 999999;
...
function authenticate(callback){
  ...
  fetch(fb+'/Users.json?auth='+ak).then(function(r){if(!r.ok)throw new Error('Status '+r.status);return r.json()}).then(function(users){
    ...
    if(profile.Status==='Banned'){loading.remove();setTimeout(function(){dialog('Account Banned','Your account has been permanently suspended.\n\nContact support.',null,'bx-block','#ff4757',true)},400);return}
    if(profile.Status==='Disabled'){loading.remove();setTimeout(function(){dialog('Account Disabled','Your account has been disabled.\n\nContact an administrator.',null,'bx-minus-circle','#febc2e',true)},400);return}
    if(profile.Status==='Expired'){loading.remove();setTimeout(function(){dialog('Subscription Expired','Your subscription has expired.\n\n'+fmtT(duracao)+'\n\nRenew to continue.',[{text:'Close'}],'bx-time-five','#ff4757')},400);return}
...
function testAPI(provider,key,callback){
  ...
  var headers={};
  if(provider==='chatgpt'){headers['Authorization']='Bearer '+key}
  else if(provider==='deepseek'){headers['Authorization']='Bearer '+key}
  else if(provider==='mistral'){headers['Authorization']='Bearer '+key}
  
  fetch(url,{method:'GET',headers:headers}).then(function(r){
    callback(r.ok,'Status: '+r.status)
  }).catch(function(e){
...
function showAPI(){
  ...
  if(apiKeys.length>0){
    ...
    var testBtn=d.createElement('button');testBtn.textContent='Test';
    ...
    testBtn.addEventListener('click',function(){
      ...
      testAPI(api.Provider,api.API,function(ok,msg){
        testBtn.textContent='Test';testBtn.style.pointerEvents='auto';
    ...
    var delBtn=d.createElement('button');delBtn.textContent='Remove';
    delBtn.style.cssText='flex:1;height:32px;border-radius:8px;font-size:11px;cursor:pointer;font-family:Inter,sans-serif;transition:0.2s;border:1px solid #2a0a0a;background:transparent;color:#ff4757;';
    ...
    delBtn.addEventListener('click',function(){
      var sel=savedSelect.value;if(!sel)return;
      var u={};u['IA/'+sel]=null;
      saveUserData(u,function(){notify('API removed','info',2000);showAPI()})
    });
